...
const ListItems = (rp:ItemsType)=>{
    let {array,selected,isPopUp, itemSelected} = rp,
        list = isPopUp ? [...array].splice(0,MAX_ITEMS_IN_LIST) : [...array];
    return list.map((ex,k)=><Items popUp={isPopUp} onClick={(e)=>{console.log(k);itemSelected(k)}} className={selected === k  && isPopUp ? 'selected' : 'none'} key={k}>
...
class List extends Component<any>{
    ...
    render(){
        let toggle = this.props.toggleBlock ? this.props.toggleDropList.open : true,
